...
Execute os serviços em containers Docker (mais fácil e isolado):
...
```bash
# Windows PowerShell
.\docker.ps1 build     # Build (primeira vez)
.\docker.ps1 start     # Iniciar
.\docker.ps1 logs      # Ver logs
.\docker.ps1 test      # Testar serviços
.\docker.ps1 stop      # Parar

# Linux/Mac
chmod +x docker.sh
./docker.sh build      # Build (primeira vez)
./docker.sh start
```
...
### Option 2: Ambiente Virtual Python

Setup tradicional com ambiente virtual:
